...
from cifar10_models.densenet import densenet121 # Import the teacher model
...
D_PATCH_VALUE = 96 # New dynamic patch dimension
NUM_LAYERS = 2     # Number of QNN layers
# --- End Model Configuration ---

# --- Knowledge Distillation Hyperparameters ---
KD_ALPHA = 0.7  # Weight for the distillation loss (e.g., 0.7 means 70% soft, 30% hard)
KD_TEMPERATURE = 4.0  # Temperature for softmax in distillation
# --- End KD Hyperparameters ---
...
def init_params(S, n, Denc, D, num_layers, d_patch_config, key):
    # key = jax.random.PRNGKey(42) # Key is now an argument
    # d_patch_config is the target embedding dimension for the transformer blocks
    # input_patch_dim is the dimension from create_patches (4*4*3 = 48)
    input_patch_dim = 4*4*3 
    d_ffn = d_patch_config * 4  # Inner dimension of FFN, based on d_patch_config

    # Key splitting: 
    # Patch embed (w,b) (2) + Pos enc (1) + num_layers * (Q,K,V,FFN_w1,FFN_w2) (5*L) + final head (1 for weight)
    # Total random keys needed: 2 + 1 + num_layers * 5 + 1
    num_random_keys = 2 + 1 + num_layers * 5 + 1
    keys = jax.random.split(key, num_random_keys)
    key_idx = 0

    # Parameters for patch embedding projection
    ...
    key_idx += 1
    patch_embed_b = jnp.zeros((d_patch_config,), dtype=jnp.float32) # Small random bias
    key_idx += 1
    ...
    for i in range(num_layers):
        layer_params = {
            ...
            # Kaiming (He) initialization for FFN layers (followed by ReLU)
            'ffn_w1': jax.random.normal(keys[key_idx+3], (d_patch_config, d_ffn), dtype=jnp.float32) * jnp.sqrt(2.0 / d_patch_config),
            'ffn_b1': jnp.zeros((d_ffn,), dtype=jnp.float32),
            'ffn_w2': jax.random.normal(keys[key_idx+4], (d_ffn, d_patch_config), dtype=jnp.float32) * jnp.sqrt(2.0 / d_ffn), # Fan-in for ffn_w2 is d_ffn
...
def train_qvit(n_train, n_test, n_epochs, batch_size=64):
    ...
    # CIFAR-10 mean and std for student model's input preprocessing (denormalization part)
    student_cifar_mean_np = np.array([0.4914, 0.4822, 0.4465]).reshape(1, 3, 1, 1)
    student_cifar_std_np = np.array([0.2470, 0.2435, 0.2616]).reshape(1, 3, 1, 1)

    # Initialize Student Model and Parameters
    student_model = QSANN_image_classifier(S=64, n=5, Denc=2, D=1, num_layers=NUM_LAYERS, d_patch_config=D_PATCH_VALUE)
    student_key = jax.random.PRNGKey(42) # Key for student parameters
    student_params = init_params(S=64, n=5, Denc=2, D=1, num_layers=NUM_LAYERS, d_patch_config=D_PATCH_VALUE, key=student_key)
    
    # Count and print student parameters
    total_student_params = count_parameters(student_params)
    print(f"\nTotal number of parameters in the QViT (Student) model: {total_student_params:,}")

    # Load Pre-trained Teacher Model (DenseNet121)
    print("Loading pre-trained DenseNet121 teacher model...")
    teacher_model = densenet121(pretrained=True)
    teacher_model.eval() # Set to evaluation mode
    # If CUDA is available and you want to use it for the teacher (PyTorch part):
    ...
    optimizer = optax.chain(
        ...
        optax.adamw(learning_rate=lr_schedule, weight_decay=1e-4) # You reverted to adam, keeping it here
    )
    # If you wanted AdamW: optax.adamw(learning_rate=lr_schedule, weight_decay=1e-4)
    ...
    def distillation_loss_fn(student_params_train, student_input_patched, teacher_logits_frozen, y_batch_hard_labels):
        student_logits = student_model(student_input_patched, student_params_train)

        # Hard Loss (Student vs. Hard Labels)
        hard_loss = jnp.mean(softmax_cross_entropy_with_integer_labels(student_logits, y_batch_hard_labels))

        # Soft Loss (Student vs. Teacher)
        # teacher_logits_frozen is already from the teacher and should not require gradients
        soft_loss = jnp.mean(optax.kl_divergence(
            jax.nn.log_softmax(student_logits / KD_TEMPERATURE, axis=-1),
            jax.nn.softmax(teacher_logits_frozen / KD_TEMPERATURE, axis=-1)
        )) * (KD_TEMPERATURE ** 2) # Scale by T^2 as is common practice

        combined_loss = (1.0 - KD_ALPHA) * hard_loss + KD_ALPHA * soft_loss
        return combined_loss, student_logits

    # JIT-compiled update step for a single batch with KD
    @jax.jit
    def update_batch_kd(student_params_train, opt_state_train, 
                        student_input_patched, teacher_logits_frozen, y_batch_hard_labels):
        (loss_val, student_logits), grads = jax.value_and_grad(distillation_loss_fn, argnums=0, has_aux=True)(
            student_params_train, student_input_patched, teacher_logits_frozen, y_batch_hard_labels
        )
        updates, new_opt_state = optimizer.update(grads, opt_state_train, student_params_train)
        new_student_params = optax.apply_updates(student_params_train, updates)
        batch_acc = accuracy_multiclass(student_logits, y_batch_hard_labels)
        return new_student_params, new_opt_state, loss_val, batch_acc

    current_student_params = student_params
    ...
    for epoch in range(n_epochs):
        ...
        for x_batch_torch, y_batch_torch in train_loader:
            # Teacher model forward pass (PyTorch)
            # x_batch_torch is already normalized as per CIFAR-10 torchvision standards, which densenet121 expects
            with torch.no_grad(): # Ensure no gradients for teacher
                teacher_input_torch = x_batch_torch.to(device) # If using GPU for teacher
                teacher_logits_torch = teacher_model(teacher_input_torch)
            teacher_logits_jax = jnp.array(teacher_logits_torch.cpu().numpy())

            # Student model input preparation
            x_batch_np_for_student = x_batch_torch.numpy() # This is (B, C, H, W) normalized by PyTorch's ToTensor and Normalize
            # Denormalize using student's dataset mean/std (which are the same as PyTorch's for CIFAR10)
            x_batch_denorm_np = (x_batch_np_for_student * student_cifar_std_np) + student_cifar_mean_np
            x_batch_denorm_np = np.clip(x_batch_denorm_np, 0., 1.)
            x_batch_transposed_np = x_batch_denorm_np.transpose(0, 2, 3, 1)  # (B,H,W,C)
            x_student_patches_np = create_patches(x_batch_transposed_np)
            x_for_student_jax = jnp.array(x_student_patches_np)
            
            y_for_student_jax = jnp.array(y_batch_torch.numpy().reshape(-1, 1))
            
            current_student_params, current_opt_state, batch_loss, batch_acc = update_batch_kd(
                current_student_params, current_opt_state, 
                x_for_student_jax, teacher_logits_jax, y_for_student_jax
            )
        ...
        test_loss, test_acc = evaluate(student_model, current_student_params, test_loader)
...
train_sizes = [128] # Consider smaller sizes first for 10-class
