...
def train_qvit(n_train, n_test, n_epochs, batch_size=64):
    ...
    model = QSANN_image_classifier(S=64, n=5, Denc=2, D=1, num_layers=NUM_LAYERS, d_patch_config=D_PATCH_VALUE)
    params = init_params(S=64, n=5, Denc=2, D=1, num_layers=NUM_LAYERS, d_patch_config=D_PATCH_VALUE,key=jax.random.PRNGKey(42))
    
    # Count and print total parameters
    total_params = count_parameters(params)
    print(f"\nTotal number of parameters in the QViT model: {total_params:,}")
    # For reference:
    # - Classical ViT-Tiny (patch 16, 224px images, ~192 embed_dim, 12 layers, 3 heads) is ~5.7M parameters.
    # - ViT-Small is ~22M parameters.
    # - ViT-Base is ~86M parameters.
    # This QViT model is expected to be significantly smaller, especially with num_layers=1.
