...
class QSAL_pennylane:
    ...
    def __call__(self, input, params_Q, params_K, params_V):
        ...
        S = self.seq_num
        d = self.d
        # input = (input - input.mean(axis=-1, keepdims=True)) / (input.std(axis=-1, keepdims=True) + 1e-5)
...
plt.tight_layout()
plt.savefig('qvit_learning_curves.png')
plt.close()

# Save results to CSV
results_df.to_csv('qvit_results.csv', index=False)
print("Results saved to qvit_results.csv")
print("Plots saved to qvit_learning_curves.png")