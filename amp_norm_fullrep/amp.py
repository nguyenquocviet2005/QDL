...
class QSANN_text_classifier(nn.Module):
    def __init__(self, S, n, Denc, D, num_layers):
        ...
        # self.layer_norm_2 = nn.LayerNorm(self.d*S)
...
def run_iterations(n_train):
    ...
    results_df = pd.DataFrame(
        columns=["n_train","train_acc", "train_cost", "test_acc", "test_cost", "step"]
    )
...
try:
    with open(output_file, 'x') as f:
        pd.DataFrame(columns=["n_train","train_acc", "train_cost", "test_acc", "test_cost", "step"]).to_csv(f, index=False)
