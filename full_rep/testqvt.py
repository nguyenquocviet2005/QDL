...
class QSAL_pennylane(nn.Module):
    def __init__(self, S, n, Denc, D):
        ...
        self.init_params_Q = nn.Parameter(
            torch.stack([(np.pi/4) * (2 * torch.randn(n*(D+2)) - 1) for _ in range(S)]).to(device)
        )
