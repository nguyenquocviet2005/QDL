...
class QSAL_pennylane(nn.Module):
    ...
    def circuit_v(self, inputs, weights):
        # Pad or truncate inputs to size 2**num_q
        if self.d < 2**self.num_q:
            padded_inputs = torch.cat([inputs, torch.zeros(2**self.num_q - self.d, device=inputs.device)])
        else:
            padded_inputs = inputs[:2**self.num_q]  # Truncate if d > 2^n
        # Normalize the input vector
        norm = torch.linalg.norm(padded_inputs)
        if norm > 0:
            padded_inputs = padded_inputs / norm
        else:
            # Handle zero norm (rare in practice) with uniform state
            padded_inputs = torch.ones_like(padded_inputs) / torch.sqrt(torch.tensor(2**self.num_q, dtype=inputs.dtype, device=inputs.device))
        # Encode input into amplitudes
        qml.AmplitudeEmbedding(padded_inputs, wires=range(self.num_q))
        # Apply the learnable ansatz
        idx = 0
        ...
        for i in range(self.D):
            ...
            for j in range(self.num_q):
                ...
                idx += 1
        # Return expectation values of d observables
    ...
    def circuit_qk(self, inputs, weights):
        ...
        if norm > 0:
            ...
            # Handle zero norm with uniform state
            padded_inputs = torch.ones_like(padded_inputs) / torch.sqrt(torch.tensor(2**self.num_q, dtype=inputs.dtype, device=inputs.device))
        # Encode input into amplitudes
        qml.AmplitudeEmbedding(padded_inputs, wires=range(self.num_q))
        # Apply the learnable ansatz
    ...
    def forward(self, input):
        ...
        # Apply layer normalization at the beginning
        # normed_input = self.layer_norm(input)  # Shape: (batch_size, S, d)
...
class QSANN_text_classifier(nn.Module):
    def __init__(self, S, n, Denc, D, num_layers):
        ...
        self.d= n * (Denc + 2)  # Dimension of input/output vectors
        self.final_layer = nn.Linear(self.d * S, 1).float()
...
def load_digits_data(n_train, n_test):
    """
    Load and preprocess digits dataset for digits 0 and 1 with per-pixel normalization.
    """
    # Load the dataset and filter for digits 0 and 1
    digits = load_digits()
    X, y = digits.data, digits.target
    mask = (y == 0) | (y == 1)  # Filter for digits 0 and 1
    X, y = X[mask], y[mask]
    X = X.reshape(-1, 8, 8)  # Reshape to 8x8 images for normalization
    ...
    # Convert to tensors
    X_train = torch.tensor(X_train, dtype=torch.float32)  # Shape: (n_train, 8, 8)
    X_test = torch.tensor(X_test, dtype=torch.float32)  # Shape: (n_test, 8, 8)

    # Compute per-pixel mean and std from training data
    X_train_flat = X_train.view(n_train, -1)  # Shape: (n_train, 64)
    mean = X_train_flat.mean(dim=0)  # Shape: (64,)
    std = X_train_flat.std(dim=0)  # Shape: (64,)
    std = std.clamp(min=1e-6)  # Avoid division by zero

    # Apply per-pixel normalization
    X_train = (X_train.view(n_train, -1) - mean) / std  # Normalize training data
    X_test = (X_test.view(n_test, -1) - mean) / std    # Normalize test data using train stats

    # Reshape back to (batch_size, 8, 8)
    X_train = X_train.view(n_train, 8, 8)
    X_test = X_test.view(n_test, 8, 8)

    # Reshape to (batch_size, 4, 16) for model input
    X_train = X_train.reshape(-1, 4, 16)
    X_test = X_test.reshape(-1, 4, 16)
