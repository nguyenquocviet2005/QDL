...
class QSANN_text_classifier(nn.Module):
    def __init__(self, S, n, Denc, D, num_layers):
        ...
        self.d = n * (Denc + 2)  # Feature dimension
        self.Qnn = QSANN_pennylane(S, n, Denc, D, num_layers)
        self.final_layer = nn.Linear(self.d * S, 1).float()

        # Initialize sinusoidal positional encoding
        self.register_buffer('pos_encoding', self._get_sinusoidal_encoding(S, self.d))

    def _get_sinusoidal_encoding(self, S, d):
        """
        Compute sinusoidal positional encoding for S positions and d dimensions.
        Returns a tensor of shape (1, S, d).
        """
        position = torch.arange(S, dtype=torch.float32).unsqueeze(1)  # Shape: (S, 1)
        div_term = torch.exp(torch.arange(0, d, 2, dtype=torch.float32) * (-torch.log(torch.tensor(10000.0)) / d))  # Shape: (d/2,)
        pe = torch.zeros(S, d)  # Shape: (S, d)
        pe[:, 0::2] = torch.sin(position * div_term)  # Even indices
        pe[:, 1::2] = torch.cos(position * div_term)  # Odd indices
        return pe.unsqueeze(0)  # Shape: (1, S, d)

    def forward(self, input):
        # Add positional encoding to input
        x = input + self.pos_encoding  # Shape: (batch_size, S, d)
        x = self.Qnn(x)  # Pass through QSANN_pennylane
        x = torch.flatten(x, start_dim=1)  # Shape: (batch_size, S * d)
        return torch.sigmoid(self.final_layer(x))  # Shape: (batch_size, 1)
