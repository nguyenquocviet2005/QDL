...
class QSANN_text_classifier(nn.Module):
    def __init__(self, S, n, Denc, D, num_layers):
        ...
        self.d= n * (Denc + 2)  # Dimension of input/output vectors
        self.layer_norm_1 = nn.LayerNorm(self.d)  # Layer norm input
        self.layer_norm_2 = nn.LayerNorm(self.d)  # Layer norm before final linear
        self.final_layer = nn.Linear(self.d * S, 1).float()

    def forward(self, x):
        x = self.layer_norm_1(x)  # Apply layer norm: (batch_size, S, d)
        x = self.Qnn(x)
        x = self.layer_norm_2(x)  # Apply layer norm: (batch_size, S, d)
